...
(function () {
  ...
  function validateSettings(source, target) {
    target.controllerEnabled = source.controllerEnabled !== false;
    target.vibration = source.vibration !== false;
    target.invertLookY = source.invertLookY === true;
    target.cameraPeek = source.cameraPeek !== false;
    target.swapConfirmCancel = source.swapConfirmCancel === true;
    target.southpaw = source.southpaw === true;
    target.moveDeadzone = numberOr(source.moveDeadzone, settingDefaults.moveDeadzone, 0.02, 0.6);
    target.lookDeadzone = numberOr(source.lookDeadzone, settingDefaults.lookDeadzone, 0.02, 0.6);
    target.outerDeadzone = numberOr(source.outerDeadzone, settingDefaults.outerDeadzone, 0.6, 1);
    target.lookSensitivityX = numberOr(source.lookSensitivityX, settingDefaults.lookSensitivityX, 0.2, 3);
    target.lookSensitivityY = numberOr(source.lookSensitivityY, settingDefaults.lookSensitivityY, 0.2, 3);
    target.triggerThreshold = numberOr(source.triggerThreshold, settingDefaults.triggerThreshold, 0.05, 0.95);
    target.responseCurve = numberOr(source.responseCurve, settingDefaults.responseCurve, 1, 3);
    target.promptStyle = ['auto', 'xbox', 'keyboard'].indexOf(source.promptStyle) >= 0 ? source.promptStyle : 'auto';
  ...
  function update(dt) {
    ...
    lastPollTime = (window.performance && performance.now) ? performance.now() : lastPollTime + delta * 1000;
