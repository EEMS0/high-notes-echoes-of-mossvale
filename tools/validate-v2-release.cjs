...
assert.match(arena, /PlatformArena\.prototype\.requestAssets/, 'Echo Arena media has an explicit lazy-load boundary');
assert.match(arena, /if\(this\.assetsRequested\)return/, 'Echo Arena media is requested only once');
